    ...
    def select(self):
        ...
        for components in range(self.min_n_components, self.max_n_components + 1):
            try:
                ...
                params = components * (components - 1) + 2 * features * components
...
class SelectorDIC(ModelSelector):
    ...
    def select(self):
        ...
        for components in range(self.min_n_components, self.max_n_components + 1):
            ...
            # DIC = log(P(X(i)) - 1/(M-1)SUM(log(P(X(all but i))
            dic = LogL - (1 / (M - 1)) * (SumLogL - (0 if LogL == float("-inf") else LogL))
            if dic > best_score:
...
class SelectorCV(ModelSelector):
    ...
    def select(self):
        ...
        for components in range(self.min_n_components, self.max_n_components + 1):
            scores, splits = [], SelectorCV.splits
            hmm_model, logL = None, None
            ...
            if avg > best_score:
                best_score, best_model = avg, hmm_model
